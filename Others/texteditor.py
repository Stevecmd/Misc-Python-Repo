...
# root.iconbitmap('E:/gui/python.ico') #Edit this if you want to customize the default icon for the program
root.geometry("640x480") #control the size of the initial launch screen
...
def new_file():
	...
	my_text.delete("1.0", END) #This basically means delete from the begining to the end
	# Update status bars
...
def open_file():
	...
	# Grab Filename
	text_file = filedialog.askopenfilename(initialdir="E:/gui/", title="Open File", filetypes=(("Text Files", "*.txt"), ("HTML Files", "*.html"), ("Python Files", "*.py"), ("All Files", "*.*")))
	...
	name = name.replace("E:/gui/", "")
	root.title(f'{name} - TextPad!')

	# Open the file
	text_file = open(text_file, 'r') #r means read
...
# Save As File
def save_as_file():
	text_file = filedialog.asksaveasfilename(defaultextension=".*", initialdir="E:/gui/", title="Save File", filetypes=(("Text Files", "*.txt"), ("HTML Files", "*.html"), ("Python Files", "*.py"), ("All Files", "*.*")))
	if text_file:
		# Update Status Bars
		name = text_file
		status_bar.config(text=f'Saved: {name}        ')
		name = name.replace("E:/gui/", "")
		root.title(f'{name} - TextPad!') #Changes the title to the new file name

		# Save the file
		text_file = open(text_file, 'w') #w means write
...
def save_file():
	...
	if open_status_name:
		# Save the file
		text_file = open(open_status_name, 'w') #w means write
		text_file.write(my_text.get(1.0, END))
		# Close the file
		text_file.close()
...
def print_file():
	...
	# Grab Filename
	file_to_print = filedialog.askopenfilename(initialdir="E:/gui/", title="Open File", filetypes=(("Text Files", "*.txt"), ("HTML Files", "*.html"), ("Python Files", "*.py"), ("All Files", "*.*")))
...
edit_menu.add_command(label="Cut", command=lambda: cut_text(False), accelerator="(Ctrl+x)") #accelerator binds the keyboard shortcuts
